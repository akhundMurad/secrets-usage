...
class SecretsService:
    ...
    def create_secret(self, *, name: str, plain_value: str) -> None:
        with self._uow as uow:
            aes = AES.new(key=bytes(self._application_settings.secret_key, encoding="utf-8"), mode=AES.MODE_CBC)
            ...
            self._secrets_gateway.insert(name=name, value=b64encode(encrypted_value).decode(), iv=b64encode(aes.iv).decode())
            uow.commit()

    def get_secret_by_name(self, *, name: str) -> SecretDTO:
        with self._uow as uow:
            secret = self._secrets_gateway.find_by_name(name=name)
            if not secret:
                raise BusinessLogicError
            aes = AES.new(key=bytes(self._application_settings.secret_key, encoding="utf-8"), mode=AES.MODE_CBC, iv=bytes(secret.iv, encoding="utf-8"))
            value = b64decode(secret.value)
            decrypted = aes.decrypt(value)
            secret.value = unpad(decrypted, AES.block_size)
            uow.commit()
